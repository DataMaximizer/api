...
export class OfferEnhancementService {
    ...
    3. 3-5 key benefits as HTML list
    4. 3-5 main features as HTML list
    5. Specific target audience description
    6. 2-3 unique selling points as HTML list

    Use these HTML tags appropriately:
    - <p> for paragraphs
  ...
  `;
		...
		const completion = await this.openai.chat.completions.create({
			...
			messages: [
				{
					role: "system",
					content: `You are an e-commerce marketing expert.
        ALWAYS return content in HTML format.
        ALWAYS use appropriate HTML tags for structure.
        NEVER return raw text without HTML tags.`,
				},
		...
		const formatToHtml = (text: string) =>
			text.includes("<p>") ? text : `<p>${text}</p>`;

		const formatListToHtml = (items: string[]) =>
			items[0]?.includes("<li>")
				? items
				: [`<ul>${items.map((item) => `<li>${item}</li>`).join("")}</ul>`];

		return {
			description: formatToHtml(
				response.description || offer.description || "",
			),
			detailedDescription: formatToHtml(response.detailedDescription || ""),
			benefits: formatListToHtml(response.benefits || []),
			features: formatListToHtml(response.features || []),
			targetAudience: formatToHtml(response.targetAudience || ""),
			uniqueSellingPoints: formatListToHtml(response.uniqueSellingPoints || []),
		};
	}
