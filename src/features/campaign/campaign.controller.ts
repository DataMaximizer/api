...
export class CampaignController {
	...
	static async generateContent(
		...
		try {
			const { offerId, framework, tone, numberOfVariants = 1, format = 'html' } = req.body;

			logger.info(
				`Generating content for offer ${offerId} with framework ${framework}, tone ${tone}, and format ${format}`,
			);
